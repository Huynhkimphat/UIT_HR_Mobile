...
function DayOffLetter({navigation}) {
    ...
    const renderAttendance = ({item}) => {
        return <View key={item.id} style={styles.item}>
            <View style={{
                ...
                backgroundColor: item.isApproved ? theme['color-success-default'] : "rgb(251, 140, 0)",
                borderRadius: 5,
            }}>
                <Text category={"c2"} style={{color: "white"}}>{item.isApproved ? "APPROVED" : "PENDING"}</Text>
            </View>
...
export default React.memo(DayOffLetter);
